...
def _get_tokenizer_and_model():
    ...
    The `transformers` import and the (network-fetched) pre-trained model are
    deferred to first use, so that importing this module does not require
    those optional, heavy dependencies to be installed.
    ...
    if _tokenizer is None or _model is None:
        try:
            ...
            raise RuntimeError(
                'Icon matching requires the optional `transformers` and `torch`'
                ' dependencies, which are not installed.'
            ) from e

        _tokenizer = BertTokenizer.from_pretrained(GlobalConfig.TINY_BERT_MODEL)
        _model = BertModel.from_pretrained(GlobalConfig.TINY_BERT_MODEL)
...
def find_icons(keywords: list[str]) -> list[str]:
    ...
    try:
        ...
        raise RuntimeError(
            'Icon matching requires the optional `scikit-learn` dependency,'
