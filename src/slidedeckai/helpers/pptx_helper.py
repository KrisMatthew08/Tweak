...
def generate_powerpoint_presentation(
    ...
    subtitle.text = 'Generated with Tweak'
    all_headers = [
        title.text,
    ]
...
def _handle_icons_ideas(
    ...
    if 'bullet_points' in slide_json and slide_json['bullet_points']:
        ...
        try:
            ...
            logger.warning('Icon matching unavailable, using default fallback icon: %s', ex)
            fallback_icon_files = ['123'] * len(icons_texts)
...
def _handle_key_message(
    ...
    if 'key_message' in slide_json and slide_json['key_message']:
        ...
        # Shrink any content placeholders that would otherwise overlap the key
        # message box, and let their text shrink to fit so nothing is cut off.
        for shape in the_slide.shapes:
            if shape == the_slide.shapes.title or not shape.has_text_frame:
                continue
            if (
                shape.top is None
                or shape.left is None
                or shape.width is None
                or shape.height is None
            ):
                continue

            shape_bottom_inch = (shape.top + shape.height) * EMU_TO_INCH_SCALING_FACTOR
            if shape_bottom_inch > top.inches:
                new_height_inch = top.inches - shape.top * EMU_TO_INCH_SCALING_FACTOR - 0.1
                if new_height_inch > 0:
                    # Re-assign left/top/width too: setting only `height` on a
                    # placeholder that inherits its geometry from the layout
                    # creates a fresh transform with left/top/width defaulted
                    # to 0, collapsing the box to a sliver.
                    left_emu, top_emu, width_emu = shape.left, shape.top, shape.width
                    shape.height = pptx.util.Inches(new_height_inch)
                    shape.left, shape.top, shape.width = left_emu, top_emu, width_emu
                shape.text_frame.word_wrap = True
                shape.text_frame.auto_size = pptx.enum.text.MSO_AUTO_SIZE.TEXT_TO_FIT_SHAPE
